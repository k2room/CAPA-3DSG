...
def compute_spatial_similarities(cfg, detection_list: DetectionList, objects: MapObjectList) -> torch.Tensor:
    ...
    '''
    det_bboxes = detection_list.get_stacked_values_torch('bbox').to(device=cfg.device)
    obj_bboxes = objects.get_stacked_values_torch('bbox').to(device=cfg.device)
    ...
    if cfg.spatial_sim_type == "iou":
        ...
        spatial_sim = torch.from_numpy(spatial_sim).T.to(device=cfg.device)
    else:
...
def merge_detections_to_objects(
    ...
    row_max, row_arg = torch.max(agg_sim, dim=1)  # (M,), (M,)
    row_max = row_max.cpu()
    row_arg = row_arg.cpu()
    M = agg_sim.shape[0]

    for i in range(M):
        if row_max[i].item() == float('-inf'):
            objects.append(detection_list[i])
        else:
            j = int(row_arg[i].item())
            matched_det = detection_list[i]
            matched_obj = objects[j]
            merged_obj = merge_obj2_into_obj1(cfg, matched_obj, matched_det, run_dbscan=False)
            objects[j] = merged_obj
