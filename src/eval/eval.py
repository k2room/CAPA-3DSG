"""
Evaluation code for Functional 3DSG Prediction with CLIP-based Node Classification and SBERT-based Edge Classification

Dependencies:
    conda install -c conda-forge sentence-transformers==5.1.2
    pip install "transformers==4.35.2" open3d
...
# Focused functional part classes in SceneFun3D_Graph / FunGraph3D dataset
CLASS_LABELS_FUNC = [
...
# SceneFun3D_Graph divided & refined labels
...
# Focused functional part classes in CAPA dataset
CLASS_LABELS_FUNC_CAPA = []
...
def get_parser():
    ...
    parser.add_argument("--part_file", type=str, required=True)
...
def get_label(pred_func):
    if 'refined_obj_tag' in pred_func.keys():
        label = pred_func['refined_obj_tag']
    else:
        label = majority_name(pred_func['class_name'])
...
if __name__ == '__main__':
    ...
    if args.dataset == 'SceneFun3D':
        ...
        with open(args.root_path+'/SceneFun3D.annotations.v2.json', 'r') as f:
            gt_annos = json.load(f)
        gt_annos = [anno for anno in gt_annos if anno['scene_id'] == args.scene]
        with open(args.root_path+'/SceneFun3D.relations.v2.json', 'r') as f:
        ...
        with open(args.root_path+'/FunGraph3D.annotations.v2.json', 'r') as f:
            gt_annos = json.load(f)
        gt_annos = [anno for anno in gt_annos if anno['scene_id'] == args.scene]
        with open(args.root_path+'/FunGraph3D.relations.v2.json', 'r') as f:
            gt_edge_annos = json.load(f)
        gt_edge_annos = [anno for anno in gt_edge_annos if anno['scene_id'] == args.scene]
    else:
        exit(1)
    ...
    print(f"Loading edge file", end='...')
    with open(str(args.edge_file), "rb") as f:
        edges = pickle.load(f)
    print("Done.")

    print(f"Loading object and part files", end='...')
    objects = load_result(str(args.obj_file))
    parts = load_result(str(args.part_file))
    print("Done.")    

    print(f"Loading CLIP and SBERT models", end='...')
    ...
    all_node_embed = []
    norm_all_node_embed = None
    print(f"Computing node category list", end='...')
    if args.dataset == 'SceneFun3D':
        all_nodes = all_nodes_scenefun3d_divided
    elif args.dataset == 'FunGraph3D':
        all_nodes = all_nodes_fungraph3d_divided
    elif args.dataset == 'CAPA':
        # TODO: implement for CAPA dataset
    ...
    for i in all_nodes:
        inputs = processor(text=[i], return_tensors="pt", padding=True, truncation=True)
        with torch.no_grad():
            embeddings = model.get_text_features(**inputs).detach().cpu().numpy()
        all_node_embed.append(embeddings[0])
    all_node_embed = np.stack(all_node_embed, axis=0)
    norm_all_node_embed = all_node_embed / np.linalg.norm(all_node_embed, axis=1, keepdims=True)
    print("Done.")

    print(f"Loading edge category file", end='...')
    with open(args.root_path+'/all_edges.v2.json', 'r') as f:
        all_edges = json.load(f)
    print("Done.")

    print(f"Computing edge category embeddings", end='...')
    all_edge_embed = sbert.encode(all_edges)
    print("Done.")
    ...
    for k in ks:
        ...
        for gt_edge in tqdm(gt_edge_annos, desc=f'Edge R@{k}'):
            if args.debug: print(" ")
            try:
            ...
            for edge in edges:
                ...
                pred_label1 = get_label(pred_func)
                ...
                if iou1 > args.iou_threshold:
                    ...
                    similarity = np.dot(norm_embeddings, norm_all_node_embed.T)
                    topk_indices = np.argsort(similarity[0], axis=0)[-k:][::-1]
                    topk_label = [all_nodes[idx] for idx in topk_indices]
                    if args.debug: print(f'\tNode 1 - GT: {gt_label1}, pred: {pred_label1}, iou: {iou1:.4f}')
  
                    flag_pass = False
                    if gt_label1 in topk_label:
                        flag_pass = True
                    else:
                        for gts in gt_label1.split('/'): # handle multi-label case
                            gts = gts.strip()
                            if gts in topk_label:
                                flag_pass = True
                                break
                    if not flag_pass:
                        if args.debug: print(f"\t  GT '{gt_label1}' not in {topk_label}")
                        continue
                else:
                    continue
                ...
                pred_label2 = get_label(pred_obj)
                ...
                if iou2 > args.iou_threshold:
                    ...
                    topk_indices = np.argsort(similarity[0], axis=0)[-k:][::-1]
                    topk_label = [all_nodes[idx] for idx in topk_indices]
                    if args.debug: print(f'\tNode 2 - GT: {gt_label2}, pred: {pred_label2}, iou: {iou2:.4f}')
    
                    flag_pass = False
                    if gt_label2 in topk_label:
                        flag_pass = True
                    else:
                        for gts in gt_label2.split('/'):
                            gts = gts.strip()
                            if gts in topk_label:
                                flag_pass = True
                                break
                    if not flag_pass:
                        if args.debug: print(f"\t  GT '{gt_label2}' not in {topk_label}")
                        continue
                else:
                    continue
                ...
                pred_rel = edge[3]
                pred_rel = pred_rel.replace('_', ' ')
                pred_rel = pred_rel.replace('-', ' ')
                embeddings = sbert.encode([pred_rel])
                similarities = sbert.similarity(embeddings, all_edge_embed)
                argsorted = sorted(
                ...
                if args.debug:
                    print(f'\tEvaluating edge between "{pred_label1}" and "{pred_label2}":')
                    print(f'\t  GT    : {gt_rel}')
                    print(f'\t  Pred  : {pred_rel}')
                    print(f'\t  Predicted topk edges: {argsorted[:k]}')
                if gt_rel in topk_labels_rel:
                    if args.debug: print(f'--- O (Correctly retrieved edge!) ---\n')
                    flag_edge = True
                    rk_num += 1
                    break
                else:
                    if args.debug: print(f'--- X (Wrong edge!) ---\n')
    ...
    for k in ks:
        ...
        for gt in tqdm(gt_annos, desc=f'Node R@{k}'):
            ...
            for obj_id in obj_idx:
                ...
                similarity = np.dot(norm_emb, norm_all_node_embed.T)
                topk_idx = np.argsort(similarity[0])[-k:][::-1]
                topk_labels = [all_nodes[idx] for idx in topk_idx]
                if args.debug: print(f'\tNode 1 - GT: {gt_label}, pred: {pred_label}, iou: {iou:.4f}')

                flag_pass = False
                if gt_label in topk_labels:
                    flag_pass = True
                else:
                    for gts in gt_label.split('/'):
                        gts = gts.strip()
                        if gts in topk_labels:
                            flag_pass = True
                            break
                ...
                if flag_pass:
                    ...
                    if args.debug: print(f"\t  GT '{gt_label}' not in {topk_labels}")
