...
if __name__ == '__main__':
    ...
    def _default_paths(dataset):
        if dataset == 'SceneFun3D':
            base = Path(args.root_path) / args.split / args.scene / args.video
        else:  # FunGraph3D
            base = Path(args.root_path) / args.scene / args.video
        return (
            base / 'pcd_saves' / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0.3_bbox0.9_simsum1.2_dbscan.1_post.pkl.gz',
            base / 'part' / 'pcd_saves' / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0.15_bbox0.1_simsum1.2_dbscan.1_parts_post.pkl.gz',
            base / 'cfslam_funcgraph_edges.pkl',
        )
    ...
    default_obj_pkl, default_part_pkl, default_edge_pkl = _default_paths(args.dataset)

    objects_pkl = Path(args.objects_pkl) if args.objects_pkl else exit(1)
    print("Using objects pkl:", objects_pkl)
    parts_pkl   = Path(args.parts_pkl)   if args.parts_pkl   else exit(1)
    print("Using parts pkl:", parts_pkl)
    edges_pkl   = Path(args.edges_pkl)   if args.edges_pkl   else exit(1)
    print("Using edges pkl:", edges_pkl)    
    ...
    print('Top 1 Obj Recall: ', r1_num_obj , ' / ' , total_obj_num , ': ' , r1_num_obj / total_obj_num)
    print('Top 1 Fun Elements Recall: ', r1_num_func , ' / ' , total_func_num , ': ' , r1_num_func / total_func_num)
    print('Top 1 Overall Recall: ', (r1_num_func + r1_num_obj) / (total_obj_num + total_func_num))

    print('Summary: GT obj | GT func | R1 obj | R1 func | Rk(R3) obj | Rk(R3) func | R10 obj | R10 func')
    print(total_obj_num, total_func_num, r1_num_obj, r1_num_func, rk_num_obj, rk_num_func, r10_num_obj, r10_num_func) 