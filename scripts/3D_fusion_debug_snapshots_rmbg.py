...
def _get_obj_class_name(obj, classes):
    """Best-effort class name extraction from an object dict."""
    # 1) direct class_name field (often a list with one string)
    ...
    if 0 <= cid_int < len(classes):
        return str(classes[cid_int])
    return None


def _snapshot_exclude_set(cfg) -> set[str]:
    """Classes to exclude when SAVING snapshots/final outputs.

    - drop_wall_ceiling: convenience flag
    - snapshot_exclude_class_names: user-provided list of class names
    Matching is case-insensitive.
    """
    exclude: set[str] = set()
    if bool(getattr(cfg, "drop_wall_ceiling", False)):
        exclude.update({"wall", "ceiling"})

    extra = getattr(cfg, "snapshot_exclude_class_names", None)
    if extra is not None:
        try:
            ...
            # tolerate scalar values
            s = str(extra).strip()
            if s:
                exclude.add(s)

    return {s.lower() for s in exclude}


def _filter_objects_by_class_name(obj_list, exclude_names: set[str], classes):
    """Return a filtered MapObjectList, removing entries whose class name matches exclude_names."""
    if obj_list is None:
        return None
    if not exclude_names:
        return obj_list

    dev = getattr(obj_list, "device", None)
    filtered = MapObjectList(device=(dev if dev is not None else "cpu"))
    for i in range(len(obj_list)):
        obj = obj_list[i]
        name = _get_obj_class_name(obj, classes)
        if name is not None and name.lower() in exclude_names:
            continue
        filtered.append(obj)
    return filtered
...
def _save_snapshot(
    ...
    # Optional: exclude some semantic classes ONLY for saving.
    exclude = _snapshot_exclude_set(cfg)
    objects_save = _filter_objects_by_class_name(objects, exclude, classes)
    bg_list_save = _filter_objects_by_class_name(bg_list, exclude, classes)
    ...
    recon = {
        **meta,
        "mode": "reconstructed",
        "objects": prepare_objects_save_vis(objects_save, downsample_size=ds),
        "bg_objects": None if bg_list_save is None else prepare_objects_save_vis(bg_list_save, downsample_size=ds),
        "cfg": cfg_dict,
        "class_names": classes,
    ...
    inst = {
        **meta,
        ...
        "objects": prepare_objects_save_instcolor(objects_save, downsample_size=ds),
        "bg_objects": None if bg_list_save is None else prepare_objects_save_instcolor(bg_list_save, downsample_size=ds),
        "cfg": cfg_dict,
        "class_names": classes,
        "class_colors": class_colors,
    }
...
def _rewrite_save_path_flag_to_hydra_override() -> None:
    """Allow extra argparse-like flags while keeping Hydra overrides.

    Supported flags:
      - --save_path /path (or --save_path=/path)
      - --drop_wall_ceiling (or --remove_wall_ceiling)
          -> exclude wall/ceiling ONLY when saving snapshots/final pkls
      - --exclude_classes wall,ceiling,door
          -> exclude the listed class names ONLY when saving
    """
    argv = sys.argv
    ...
    # boolean flags
    if "--drop_wall_ceiling" in argv:
        argv.remove("--drop_wall_ceiling")
        argv.append("++drop_wall_ceiling=true")
    if "--remove_wall_ceiling" in argv:
        argv.remove("--remove_wall_ceiling")
        argv.append("++drop_wall_ceiling=true")

    # list flag: comma-separated
    excl = _pop_kv("--exclude_classes")
    if excl is not None:
        items = [s.strip() for s in str(excl).split(",") if s.strip()]
        if len(items) > 0:
            argv.append(f"++snapshot_exclude_class_names=[{','.join(items)}]")
...
def main(cfg: DictConfig):
    ...
    if getattr(cfg, "save_pcd", False):
        ...
        exclude = _snapshot_exclude_set(cfg)
        objects_save = _filter_objects_by_class_name(objects, exclude, classes)
        bg_save = _filter_objects_by_class_name(bg_objects, exclude, classes)
        ...
        recon_post = {
            ...
            "objects": objects_save.to_serializable(),
            "bg_objects": None if bg_save is None else bg_save.to_serializable(),
            "cfg": cfg_dict,
            "class_names": classes,
            "class_colors": class_colors,
        }
        ...
        inst_post = {
            ...
            "objects": prepare_objects_save_instcolor(objects_save, downsample_size=float(getattr(cfg, "snapshot_downsample_size", 0.01))),
            "bg_objects": None if bg_save is None else prepare_objects_save_instcolor(bg_save, downsample_size=float(getattr(cfg, "snapshot_downsample_size", 0.01))),
            "cfg": cfg_dict,
            "class_names": classes,
            "class_colors": class_colors,
        }
...
"""
python scripts/3D_fusion_debug_snapshots.py \
  scene_id=... dataset=... save_folder_name=... \
  --save_path /tmp/fusion_debug \
  --drop_wall_ceiling
...
python scripts/3D_fusion_debug_snapshots.py \
  scene_id=... dataset=... save_folder_name=... \
  --save_path /tmp/fusion_debug \
  --exclude_classes wall,ceiling,floor
"""

"""
env OMP_NUM_THREADS=8 MKL_NUM_THREADS=8 OPENBLAS_NUM_THREADS=8 NUMEXPR_NUM_THREADS=8 OMP_DYNAMIC=FALSE MKL_DYNAMIC=FALSE python scripts/3D_fusion_debug_snapshots_rmbg.py \
  scene_id=4livingroom/video1 dataset=FunGraph3D save_folder_name=CAPA_1 \
  --save_path /tmp/fusion_debug_rmbg \
  --exclude_classes wall,ceiling
"""