...
def main(cfg: DictConfig):  
    ...
    spatial_obj = (str(cfg.dataset) == "ReplicaSSG")
    ...
    # check all object pairs for generating spatial relations
    ...
    def _add_rel(obj_id, rel, subj_id):
        ...
        obj_id = int(str(obj_id).split("_")[1])
        subj_id = int(str(subj_id).split("_")[1])
        lst = spatial[subj_id]
        pair = [obj_id, rel]
        if pair not in lst:
            lst.append(pair)
    ...
    def _add_rel_unordered(id1, id2, rel):
        # store once ignoring direction: always under the lexicographically smaller node id
        if id1 < id2:
            _add_rel(id2, rel, id1)
        else:
            _add_rel(id1, rel, id2)
    ...
    for A in nodes_all:
        for B in nodes_all:
            ...
            cA = (np.asarray(mnA) + np.asarray(mxA)) * 0.5
            cB = (np.asarray(mnB) + np.asarray(mxB)) * 0.5
            center_dist = float(np.linalg.norm(cA - cB))

            if center_dist > TH_NEAR:
                continue  # skip very far pairs
            ...
            # z-axis separation (A relative to B)
            sep_z_above = float(mnA[2] - mxB[2])  # A bottom above B top if positive

            # side contact (against / attached to)
            ...
            side_contact_x = ((-TOUCH <= gap_x1 <= TOUCH) or (-TOUCH <= gap_x2 <= TOUCH)) and (y_overlap > 0) and (z_overlap > 0) and (y_ratio >= 0.2) and (z_ratio >= 0.2)
            side_contact_y = ((-TOUCH <= gap_y1 <= TOUCH) or (-TOUCH <= gap_y2 <= TOUCH)) and (x_overlap > 0) and (z_overlap > 0) and (x_ratio >= 0.2) and (z_ratio >= 0.2)
            side_contact = side_contact_x or side_contact_y

            cand_in  = _contains(mnA, mxA, mnB, mxB, tol=TOL_IN)
            cand_attached = side_contact and (z_ratio >= 0.5) and ((x_ratio >= 0.5) or (y_ratio >= 0.5))
            # (A on B): contact/support plausible (touching in z + sufficient XY overlap + A is higher)
            cand_on = (xy_area > 0) and (abs(sep_z_above) <= TOUCH) and (cA[2] > cB[2])
            # (A above B): higher without support/contact
            cand_above = (xy_area > 0) and (TOUCH < sep_z_above <= TH_NEAR)
            # "with": co-located on similar height without clear contact/containment/vertical stacking
            cand_with = (box_dist <= 0.40) and (abs(float(cA[2] - cB[2])) <= 0.40) and (overlap_ratio_xy <= 0.3)
            # "near": fallback proximity (only when no stronger geometric evidence)
            cand_near = (center_dist <= TH_NEAR) # cand_near = (box_dist <= TH_NEAR)

            # assign candidates (Object A -> Object B)
            if cand_in:
                _add_rel(B, "in", A)
            if cand_attached:
                _add_rel(B, "attached to", A)
            if cand_on:
                _add_rel(B, "on", A)
            if cand_above:
                _add_rel(B, "above", A)
