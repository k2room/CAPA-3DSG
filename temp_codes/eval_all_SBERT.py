...
if __name__ == '__main__':
    ...
    if args.obj_file == '' or args.part_file == '':            
        if args.dataset == 'SceneFun3D':
            objects = load_result(str(Path(args.root_path) / args.split / args.scene / args.video / args.gpt / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0_updated.3_bbox0.9_simsum1.2_dbscan.1_post.pkl.gz'))
            parts = load_result(str(Path(args.root_path) / args.split / args.scene / args.video / args.gpt / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0_updated.15_bbox0.1_simsum1.2_dbscan.1_parts_post.pkl.gz'))
        elif args.dataset == 'FunGraph3D':
            objects = load_result(str(Path(args.root_path) / args.scene / args.video / args.gpt / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0_updated.3_bbox0.9_simsum1.2_dbscan.1_post.pkl.gz'))
            parts = load_result(str(Path(args.root_path) / args.scene / args.video / args.gpt / 'full_pcd_ram_withbg_allclasses_overlap_maskconf0_updated.15_bbox0.1_simsum1.2_dbscan.1_parts_post.pkl.gz'))
    else:
    ...
    for k in ks:
        ...
        for gt_edge in tqdm(gt_edge_annos):
            ...
            for edge in edges:
                ...
                pred_label1 = pred_func['refined_obj_tag'].replace('_', ' ')
                pred_label1 = pred_label1.replace(':', ' ')
                ...
                if iou1 > args.iou_threshold:
                    ...
                    if args.debug: print(f'label1 - GT: {gt_label1}, pred: {pred_label1}, iou: {iou1}')
                    # if gt_label1 not in topk_label:
                    #     if args.debug: print(f"\t{gt_label1} not in {topk_label}")
                    #     continue
                    ...
                    if gt_label1 in topk_label:
                        ...
                        for gts in gt_label1.split('/'):
                            gts = gts.strip()
                            for tls in topk_label:
                                if gts in tls:
                                    flag_pass = True
                                    break
                                if tls in gts:
                ...
                pred_label2 = pred_obj['refined_obj_tag'].replace('_', ' ')
                pred_label2 = pred_label2.replace(':', ' ')
                ...
                if iou2 > args.iou_threshold:
                    ...
                    topk_indices = np.argsort(similarity[0], axis=0)[-k:][::-1]
                    topk_label = [all_labels[idx] for idx in topk_indices]
                    if args.debug: print(f'label2 - GT: {gt_label2}, pred: {pred_label2}, iou: {iou2}')
                    # if gt_label2 not in topk_label:
                    #     if args.debug: print(f"\t{gt_label2} not in {topk_label}")
                    #     continue
